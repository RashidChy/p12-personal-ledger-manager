...
The full installed tree (272 packages, direct + transitive, runtime + build-time) was
inventoried from `node_modules` and every `license` field checked:
...
| BSD-2-Clause | 9 | yes |
| BSD-3-Clause | 3 | yes |
| ISC / MIT-0 / Python-2.0 / BlueOak-1.0.0 / CC-BY-4.0 | 4 | yes (all permissive) |
...
```bash
npm ls --all
node -e "…"   # see the inventory command recorded in README.md → Licensing
```
...
| `eng.traineddata.gz` (`public/ocr/lang/`) | Apache-2.0 | https://github.com/tesseract-ocr/tessdata_fast (served by https://tessdata.projectnaptha.com/4.0.0_fast/) |
